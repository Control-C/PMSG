...
This project integrates the **PMSG (Prototype Modular Smart Glasses)** platform—powered by a **Seeed Studio XIAO ESP32-C6**—with the [Pollen Robotics Reachy Mini](https://github.com/pollen-robotics/reachy_mini) robot. 

The glasses track your head movements using an onboard I2C IMU (X, Y, Z axes) and stream data via **BLE Gamepad emulation** or a **Wi-Fi JSON API**. It also features environmental sensing (UV/Lux), haptic feedback (Vibration), and a 4-pixel addressable RGB LED status bar.

---

## 🚀 Features

*   **Dual Mode Control**: Control Reachy Mini wirelessly via native BLE HID Gamepad or a Wi-Fi HTTP API.
*   **3-Axis Head Tracking**: Reads raw I2C sensor data to map head Pitch, Roll, and Yaw to robot movements.
...
void loop() {
  ...
  // Serial Debug Output
  Serial.printf("Pitch: %.2f | Roll: %.2f | Yaw: %.2f | Lux: %.1f | UV: %d\n", pitch, roll, yaw, lux, uv);
  delay(50);
}
```
...
### Option B: Core Python SDK Integration (Wi-Fi API)
If you are developing custom software via the `reachy_mini` Python framework, parse the smart glasses API directly from your Mac script:
...
with ReachyMini() as mini:
    while True:
        try:
            ...
            response = requests.get(f"{GLASSES_IP}/api/telemetry", timeout=0.1).json()
            head_data = response["head"]
            env_data = response["environment"]

            # 2. Extract Z (Yaw), X (Roll), and Y (Pitch) tracking data
            target_pitch = head_data["pitch"]
            target_roll = head_data["roll"]

            # Trigger haptic buzz if light levels get too bright!
            if env_data["lux"] > 2000.0:
